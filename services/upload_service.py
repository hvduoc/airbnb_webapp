...
class UploadService(BaseService):
    ...
    def process_upload_files(self, files: List[UploadFile], room_mapping_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("upload_csv"):
                    return self.error_response("Insufficient permissions to upload CSV files", 403)
            ...
            for upload_file in files:
                file_summary = self._process_single_file(upload_file, airbnb_channel, room_mapping_data)
            ...
            self.log_activity("process_upload_files", {
            ...
            return self.success_response("Files processed successfully", {
                "summaries": summaries,
                "totals": {
    ...
    def _process_single_file(self, upload_file: UploadFile, airbnb_channel: Channel, room_mapping_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    row_result = self._process_csv_row(row, df, airbnb_channel, room_mapping_data)
    ...
    def _process_csv_row(self, row: pd.Series, df: pd.DataFrame, airbnb_channel: Channel, room_mapping_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _extract_booking_data(self, row: pd.Series, df: pd.DataFrame, room_mapping_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _process_building_and_property(self, booking_data: Dict) -> Tuple[Optional[Building], Optional[Property]]:
        ...
        building_code = building_code_from_name(building_name) if building_name else None
        unit_short = unit_short_from_unit_number_auto(unit_number) if unit_number else None
        property_short = f"{building_code}-{unit_short}" if (building_code and unit_short) else None
    ...
    def _upsert_booking(self, confirmation_code: str, booking_data: Dict, 
                       property_obj: Optional[Property], airbnb_channel: Channel) -> str:
        ...
        if existing:
            ...
            existing.property_id = property_obj.id if property_obj else existing.property_id
            existing.channel_id = airbnb_channel.id if airbnb_channel else existing.channel_id
            ...
            existing.booking_date = booking_data["booking_date"] or existing.booking_date
            ...
            existing.guest_contact = booking_data["guest_contact"] or existing.guest_contact
    ...
    def validate_csv_preview(self, upload_file: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            column_analysis = {}
            logical_columns = ["confirmation_code", "start_date", "end_date", "listing", "income", "guest_name"]
            ...
            for logical_col in logical_columns:
                ...
                column_analysis[logical_col] = {
                    ...
                    "sample_values": df[mapped_col].head(3).tolist() if mapped_col else []
                }
            
            return self.success_response("CSV preview generated", {
                "total_rows": len(df),
    ...
    def get_upload_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.success_response("Upload statistics retrieved", {
                "total_bookings": total_bookings,
                "by_channel": [{"channel": stat[0], "count": stat[1]} for stat in channel_stats]
            })
            
        except Exception as e:
            return self.error_response(f"Failed to get upload statistics: {str(e)}", 500)