...
from models import Booking, Channel, ExpenseCategory, ExtraCharge, Property, Salesperson
...
class RevenueService(BaseService):
    ...
    def revenue_by_property(
        ...
        if not include_cancelled:
            query = query.where(
                Booking.status.in_(["confirmed", "completed", "checked_in"])
            )
        ...
        return {
            ...
            "summary": {
                ...
                "avg_revenue_per_property": total_revenue / len(revenue_data)
                if revenue_data
                else 0,
                "period": {
    ...
    def compute_monthly_report(
        self, start_date: date, end_date: date, group_by: str
        ...
        try:
            ...
            results, monthly_channel_data = self._process_bookings(
                bookings, props, chans, sales_map, start_date, end_date, group_by
            )
            ...
            self._ensure_complete_dataset(
                results, props, chans, sales_map, start_date, end_date, group_by
            )

            # Generate final report rows
            rows = self._generate_report_rows(
                results, props, bookings, chans, sales_map, group_by
            )
            ...
            charts = self._generate_chart_data(
                results,
                self._calculate_channel_totals(bookings, chans),
    ...
    def _is_valid_booking(self, booking: Booking) -> bool:
        ...
        return bool(
            ...
            and booking.num_nights
            and booking.total_payout_vnd is not None
        )

    def _get_group_value(
        self, booking: Booking, props: Dict, chans: Dict, sales_map: Dict, group_by: str
    ) -> str:
        ...
        if group_by == "building":
            ...
            return (
                chans.get(booking.channel_id).channel_name
                if booking.channel_id in chans
                else "N/A"
            )
    ...
    def _generate_report_rows(
        ...
        for (mk, group_val), agg in sorted(results.items()):
            ...
            active_props = self._get_active_properties_count(
                group_val, props, bookings, chans, sales_map, group_by
            )
            avail = (
                active_props * calendar.monthrange(mk.year, mk.month)[1]
            )  # days in month
    ...
    def _get_active_properties_count(
        self,
        group_val: str,
        props: Dict,
        bookings: List,
    ...
    def _calculate_channel_totals(
        self, bookings: List[Booking], chans: Dict
    ) -> Dict[str, float]:
    ...
    def _generate_chart_data(
        self, results: Dict, channel_totals: Dict, monthly_channel_data: Dict
    ) -> Dict[str, Any]:
        ...
        for mk, _ in sorted_trend:
            ...
            airbnb_rev = sum(
                rev for chan, rev in month_channels.items() if "airbnb" in chan.lower()
            )
            offline_rev = sum(
                rev for chan, rev in month_channels.items() if chan.lower() == "offline"
            )
    ...
    def get_monthly_summary(self, month: date) -> Dict[str, Any]:
        ...
        end_date = start_date.replace(month=start_date.month % 12 + 1) - timedelta(
            days=1
        )
        ...
        return {
            ...
            "average_adr": totals["total_revenue"]
            // max(totals["total_sold_nights"], 1),
        }
