...
class PropertyService(BaseService):
    ...
    def create_building(
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("building_create"):
                    return self.error_response(
                        "Insufficient permissions to create building", 403
                    )
            ...
            self.log_activity(
                "create_building",
                {"building_id": new_building.id, "building_name": building_name},
            )

            return self.success_response(
                "Building created successfully", {"building_id": new_building.id}
            )
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("property_create"):
                    return self.error_response(
                        "Insufficient permissions to create property", 403
                    )
            ...
            return self.success_response(
                "Property created successfully", {"property_id": new_property.id}
            )
