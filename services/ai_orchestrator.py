...
class MultiProviderAI:
    ...
    async def _execute_optimal(
        self, task_type: TaskType, prompt: str, max_cost: float
    ) -> Dict:
        ...
        for provider_name, capability in self.providers.items():
            if task_type in capability.strength_areas:
                ...
                cost_score = 1.0 / (
                    capability.cost_per_1k_tokens + 0.001
                )  # Lower cost = higher score
                speed_score = 1.0 / (
                    capability.response_time_avg + 0.1
                )  # Lower time = higher score
                reliability_score = capability.reliability_score

                composite_score = (
                    cost_score * 0.3 + speed_score * 0.3 + reliability_score * 0.4
                )
        ...
        if not best_provider:
            ...
            best_provider = max(
                self.providers.keys(), key=lambda p: self.providers[p].reliability_score
            )
    ...
    async def _execute_consensus(
        ...
        # Execute in parallel
        tasks = [
            self._call_provider(provider, prompt) for provider in suitable_providers
        ]
        responses = await asyncio.gather(*tasks, return_exceptions=True)

        # Analyze responses
        valid_responses = [
            (suitable_providers[i], resp)
    ...
    async def _call_openai(self, prompt: str) -> str:
        ...
        response = await openai.ChatCompletion.acreate(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert AI assistant for hospitality management.",
                },
    ...
    def _analyze_consensus(self, responses: List[Tuple[str, str]]) -> Dict:
        ...
        best_idx = min(
            range(len(responses)), key=lambda i: abs(response_lengths[i] - avg_length)
        )
...
"""
        ...
        result = await self.orchestrator.execute_task(
            TaskType.DATA_ANALYSIS, prompt, strategy="optimal"
        )

        return result

    async def optimize_pricing(
        self, property_data: Dict, market_conditions: Dict
    ) -> Dict:
...
"""
    ...
    async def generate_maintenance_schedule(
        self, properties: List[Dict], issues_history: List[Dict]
    ) -> Dict:
...
"""
        ...
        result = await self.orchestrator.execute_task(
            TaskType.PLANNING, prompt, strategy="optimal"
        )
    ...
    def _summarize_booking_data(self, booking_data: List[Dict]) -> str:
        ...
        revenues = [
            b.get("total_payout_vnd", 0)
            for b in booking_data
            if b.get("total_payout_vnd")
        ]
