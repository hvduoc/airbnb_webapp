...
class GoogleSheetsService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.warning(
                f"Google Sheets service unavailable, running in demo mode: {e}"
            )
            self.spreadsheet = None
        ...
        try:
            ...
            def _create_sheets():
                ...
                if self.config.DASHBOARD_SHEET not in existing_sheets:
                    sheet = self.spreadsheet.add_worksheet(
                        title=self.config.DASHBOARD_SHEET, rows=100, cols=10
                    )
                    sheet.insert_row(["Metric", "Value", "Date", "Details"], 1)
    ...
    async def add_payment(self, payment_data: Dict[str, Any]) -> bool:
        ...
        if not GOOGLE_SHEETS_AVAILABLE or not self.spreadsheet:
            logger.info(
                f"Demo mode: Payment recorded - {payment_data.get('booking_id')}"
            )
            return True
    ...
    async def add_cashflow(self, cashflow_data: Dict[str, Any]) -> bool:
        ...
        if not GOOGLE_SHEETS_AVAILABLE or not self.spreadsheet:
            logger.info(
                f"Demo mode: Cashflow recorded - {cashflow_data.get('transaction_type')}"
            )
            return True
        ...
        try:
            ...
            logger.info(
                f"Cashflow added successfully: {cashflow_data.get('transaction_type')}"
            )
            return result
    ...
    async def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_due = sum(float(p.get("Amount Due (VND)", 0)) for p in payments)
