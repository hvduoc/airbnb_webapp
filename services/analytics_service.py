...
from models import Booking, Property, Expense, ExpenseCategory
...
class AnalyticsService(BaseService):
    ...
    def get_revenue_vs_expense_dashboard(
        ...
        revenue_data = revenue_service.revenue_by_property(start_date, end_date, include_cancelled=False)
        ...
        for property_id in all_property_ids:
            ...
            combined_properties.append({
                ...
                "property_name": revenue_info.get("property_name") or expense_info.get("property_name", f"Property {property_id}"),
                "revenue": revenue,
                "expenses": expenses,
                "profit": profit,
        ...
        top_property_margin = combined_properties[0]["margin"] if combined_properties else 0
        
        # Prepare chart data
        property_names = [p["property_name"][:15] + "..." if len(p["property_name"]) > 15 else p["property_name"] for p in combined_properties[:10]]
    ...
    def get_occupancy_metrics(
        ...
        query = select(
            ...
            Booking.__table__.join(Property.__table__, Booking.property_id == Property.id)
        ).where(
            and_(
                Booking.start_date >= start_date.strftime("%Y-%m-%d"),
                Booking.end_date <= end_date.strftime("%Y-%m-%d"),
                Booking.status.in_(["confirmed", "checked_in", "checked_out"])  # Exclude cancelled
            )
        ).group_by(
        ...
        for row in results:
            ...
            properties.append({
                ...
                "avg_stay_length": round(float(row.avg_stay_length), 2) if row.avg_stay_length else 0
            })
        ...
        overall_occupancy = (total_nights_booked / total_nights_available * 100) if total_nights_available > 0 else 0
        avg_length_of_stay = (total_nights_booked / total_bookings) if total_bookings > 0 else 0
    ...
    def get_monthly_trends(
        ...
        revenue_query = select(
            ...
            and_(
                Booking.start_date >= (end_date - timedelta(days=months_back*30)).strftime("%Y-%m-%d"),
                Booking.status.in_(["confirmed", "checked_in", "checked_out"])
            )
        ).group_by(
            func.substr(Booking.start_date, 1, 7)  # Group by YYYY-MM
        ).order_by(
            func.substr(Booking.start_date, 1, 7)
        )
        ...
        expense_query = select(
            Expense.month,
            func.sum(Expense.amount).label("monthly_expenses")
        ).where(
            Expense.month >= (end_date - timedelta(days=months_back*30)).strftime("%Y-%m")
        ).group_by(
            Expense.month
        ).order_by(
            Expense.month
        ...
        revenue_growth = ((revenue_trend[-1] - revenue_trend[0]) / revenue_trend[0] * 100) if revenue_trend[0] > 0 else 0
        expense_growth = ((expense_trend[-1] - expense_trend[0]) / expense_trend[0] * 100) if expense_trend[0] > 0 else 0
        booking_growth = ((booking_trend[-1] - booking_trend[0]) / booking_trend[0] * 100) if booking_trend[0] > 0 else 0
