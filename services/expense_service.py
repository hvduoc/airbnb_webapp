...
class ExpenseService(BaseService):
    ...
    def create_expense(self, expense_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        missing_fields = [f for f in required_fields if f not in expense_data or expense_data[f] is None]
        if missing_fields:
            raise HTTPException(
                status_code=400,
        ...
        category_query = select(ExpenseCategory).where(ExpenseCategory.id == expense_data["category_id"])
        ...
        if expense_data.get("property_id"):
            ...
            property_query = select(Property).where(Property.id == expense_data["property_id"])
    ...
    def list_expenses(
        ...
        query = select(
            ...
            Expense.__table__
            .join(ExpenseCategory.__table__, Expense.category_id == ExpenseCategory.id)
    ...
    def summary_by_property(
        ...
        query = select(
            ...
            Expense.__table__.outerjoin(Property.__table__, Expense.property_id == Property.id)
        ).group_by(
            Expense.property_id, Property.property_name
        )
        ...
        avg_expense_per_property = total_amount / total_properties if total_properties > 0 else 0
