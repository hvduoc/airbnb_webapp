...
class SalespersonService(BaseService):
    ...
    def get_all_salespeople(self, active_only: bool = False) -> List[Dict[str, Any]]:
        ...
        self.log_activity(
            "get_all_salespeople",
            {"total_count": len(result), "active_only": active_only},
        )
    ...
    def get_active_salespeople(self) -> List[Salesperson]:
        ...
        return self.session.exec(
            select(Salesperson).where(Salesperson.is_active).order_by(Salesperson.name)
        ).all()
    ...
    def create_salesperson(
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("salesperson_create"):
                    return self.error_response(
                        "Insufficient permissions to create salesperson", 403
                    )
            ...
            if commission_rate_pct < 0 or commission_rate_pct > 100:
                return self.error_response(
                    "Commission rate must be between 0 and 100 percent", 400
                )
            ...
            if existing:
                return self.error_response(
                    "A salesperson with this name already exists", 409
                )
            ...
            return self.success_response(
                ...
                {"salesperson_id": new_salesperson.id, "name": new_salesperson.name},
            )
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("salesperson_update"):
                    return self.error_response(
                        "Insufficient permissions to update salesperson", 403
                    )
            ...
            return self.success_response(
                "Salesperson updated successfully", {"salesperson_id": salesperson_id}
            )
        ...
        try:
            ...
            if not booking.salesperson_id:
                return self.success_response(
                    ...
                    {"commission_vnd": 0, "commission_rate": 0},
                )
    ...
    def get_salesperson_performance(
        ...
        try:
            ...
            # Average metrics
            avg_booking_value = (
                int(total_revenue / total_bookings) if total_bookings > 0 else 0
            )
            avg_nights_per_booking = (
                round(total_nights / total_bookings, 1) if total_bookings > 0 else 0
            )
            ...
            return self.success_response(
                "Performance data retrieved",
                {
                    "salesperson": {
                        "id": salesperson.id,
                        "name": salesperson.name,
                        "commission_rate_pct": round(
                            salesperson.commission_rate * 100, 2
                        ),
                    },
                    "metrics": {
    ...
    def get_top_performers(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_salespeople = sorted(
            all_salespeople, key=lambda x: x["total_commission_vnd"], reverse=True
        )

        return sorted_salespeople[:limit]

    def assign_booking_to_salesperson(
        self, booking_id: int, salesperson_id: Optional[int]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.current_user:
                if not self.check_permission("booking_assign"):
                    return self.error_response(
                        "Insufficient permissions to assign bookings", 403
                    )
            ...
            if salesperson_id is not None:
                ...
                if not salesperson.is_active:
                    return self.error_response(
                        "Cannot assign to inactive salesperson", 400
                    )
            ...
            return self.success_response(
                f"Booking {action} successfully",
                {"booking_id": booking_id, "salesperson_id": salesperson_id},
            )
