...
class AIContextManager:
    ...
    async def create_context(self, 
                           ...
                           user_id: str, 
                           project_id: str,
                           context_type: str = "general") -> ConversationContext:
...
""")
    ...
    async def _extract_knowledge(self, context: ConversationContext, message: Dict):
        ...
        if any(keyword in content.lower() for keyword in 
               ["solution", "recommendation", "important", "key insight", "discovered"]):
            
            # Create knowledge memory
            knowledge = AIMemoryStore(
                session_id=context.session_id,
                context_hash=self._hash_context(context.project_id, context.context_type),
    ...
    async def _manage_context_size(self, context: ConversationContext):
        ...
        total_content = " ".join([msg["content"] for msg in context.conversation_history])
    ...
    async def _load_relevant_memories(self, 
                                    project_id: str, 
                                    context_type: str, 
        ...
        stmt = select(AIMemoryStore).where(
        ...
        ).order_by(AIMemoryStore.importance_score.desc()).limit(limit)
    ...
    async def _load_project_context(self, project_id: str) -> Dict:
        ...
        return {
            "project_info": {
                ...
                "components": ["FastAPI", "Services Layer", "AI Engine", "Vue.js Frontend"],
                "key_metrics": {
                    "properties": "multiple",
    ...
    def _calculate_importance(self, content: str) -> float:
        ...
        importance_keywords = [
            "solution", "error", "bug", "fix", "optimization", "performance",
            "security", "database", "api", "integration", "deployment"
        ]
