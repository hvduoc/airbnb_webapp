    ...
    def is_allowed(
        ...
        if identifier in self.blocked_ips:
            ...
            if current_time - block_time < block_duration:
                logger.warning(
                    f"🚫 IP {identifier} vẫn bị block (còn {block_duration - (current_time - block_time):.0f}s)"
                )
                return False
            else:
        ...
        if len(request_times) >= max_requests:
            ...
            logger.warning(
                f"🚫 Block IP {identifier} do vượt rate limit: {len(request_times)}/{max_requests}"
            )
            return False
...
class InputValidator:
    ...
    def validate_file_upload(
        cls, filename: str, content_type: str, allowed_extensions: List[str]
    ) -> Dict[str, Any]:
        ...
        if file_ext not in allowed_extensions:
            result["valid"] = False
            result["errors"].append(
                f"File extension '{file_ext}' không được phép. Cho phép: {', '.join(allowed_extensions)}"
            )
...
class PasswordPolicy:
    ...
    def validate_password(self, password: str, username: str = None) -> Dict[str, Any]:
        ...
        result = {"valid": True, "errors": [], "strength_score": 0, "suggestions": []}
        ...
        if len(password) > self.max_length:
            result["valid"] = False
            result["errors"].append(
                f"Mật khẩu không được dài quá {self.max_length} ký tự"
            )
...
class SecurityAuditLogger:
    ...
    def log_suspicious_activity(
        self, activity: str, ip_address: str, details: Dict[str, Any]
    ):
        """Log suspicious activity"""
        self.log_event(
            "SUSPICIOUS_ACTIVITY",
            {"activity": activity, "ip_address": ip_address, **details},
            "CRITICAL",
        )
