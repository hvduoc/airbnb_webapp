...
class DatabaseBackup:
    ...
    def load_config(self, config_file: str) -> Dict:
        ...
        default_config = {
            ...
            "compression": True,
            "encryption": False,
            "notification": {"enabled": False, "webhook_url": "", "email": ""},
        }
        ...
        for backup_file in sorted(self.backup_dir.glob("*.sql*")):
            ...
            if metadata_file.exists():
                try:
                    ...
                    logger.warning(
                        f"⚠️ Không đọc được metadata cho {backup_file.name}: {e}"
                    )
    ...
    def restore_backup(self, backup_path: str, target_db: str = None) -> bool:
        ...
        try:
            ...
            logger.warning(
                "⚠️ CẢNH BÁO: Đang restore database - dữ liệu hiện tại sẽ bị ghi đè!"
            )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["backup", "list", "cleanup", "schedule"],
        default="backup",
