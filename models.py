...
class User(SQLModel, table=True):
    ...
    role: str = Field(
        default="user", index=True, max_length=50
    )  # Index cho role queries
...
# ==== ENHANCED OPEX MODELS - Optimized for performance ====
    ...
    class Config:
        ...
        indexes = [
            Index(
                "idx_user_active_role", "is_active", "role"
            ),  # Cho role-based queries
            Index(
                "idx_user_login_activity", "last_login", "is_active"
            ),  # Cho analytics
        ]
...
class ExtraCharge(SQLModel, table=True):
    ...
    property_id: int = Field(
        foreign_key="property.id", index=True
    )  # Index cho property queries
    charge_name: str = Field(max_length=255)
    charge_amount: int
    charge_month: str = Field(
        index=True, max_length=7
    )  # YYYY-MM format, index cho monthly reports
    ...
    category_id: Optional[int] = Field(
        default=None, foreign_key="expense_categories.id", index=True
    )

    class Config:
        indexes = [
            Index(
                "idx_charge_property_month", "property_id", "charge_month"
            ),  # Reports theo property + tháng
            Index(
                "idx_charge_category_month", "category_id", "charge_month"
            ),  # Reports theo category + tháng
        ]
...
class Property(SQLModel, table=True):
    ...
    class Config:
        indexes = [
            Index(
                "idx_property_building_active", "building_id", "is_active"
            ),  # Active properties per building
            Index(
                "idx_property_type_price", "room_type", "price_per_night"
            ),  # Filter by type + price
            Index(
                "idx_property_search", "building_name", "room_number"
            ),  # Property search
        ]
...
class Booking(SQLModel, table=True):
    ...
    total_payout_vnd: Optional[int] = Field(
        default=None, index=True
    )  # Index cho revenue queries
    ...
    source: Optional[str] = Field(
        default="airbnb", max_length=50, index=True
    )  # "airbnb", "offline"
    channel: Optional[str] = Field(
        default="official_csv", max_length=100, index=True
    )  # "official_csv", "facebook", "zalo"
    external_ref: Optional[str] = Field(
        default=None, max_length=255
    )  # ID cho offline bookings
    ...
    )  # UUID cho import batch
    row_hash: Optional[str] = Field(
        default=None, max_length=64, unique=True, index=True
    )  # SHA-256 hash cho idempotency
    ...
    class Config:
        indexes = [
            ...
            Index(
                "idx_booking_revenue", "property_id", "booking_date", "total_payout_vnd"
            ),  # Revenue reports
            Index(
                "idx_booking_channel_status", "channel_id", "status"
            ),  # Channel performance
            Index(
                "idx_booking_monthly", "property_id", "booking_date"
            ),  # Monthly reports
            # Go-Live Import Pipeline indexes
            Index(
                "idx_booking_source_channel", "source", "channel"
            ),  # Data source tracking
            Index(
                "idx_booking_ingestion", "ingestion_id", "imported_at"
    ...
    def set_import_metadata(
        self, source: str, channel: str, ingestion_id: str, external_ref: str = None
    ):
...
class ImportLog(SQLModel, table=True):
    ...
    source: str = Field(
        default="upload-form", max_length=50, index=True
    )  # "airbnb", "offline"
    channel: str = Field(
        default="official_csv", max_length=100, index=True
    )  # "official_csv", "facebook", "zalo"
...
class ExpenseCategory(SQLModel, table=True):
    ...
    class Config:
        indexes = [
            Index(
                "idx_expense_cat_hierarchy", "parent_id", "name"
            ),  # Category hierarchy
        ]
...
class Expense(SQLModel, table=True):
    ...
    )  # Index cho monthly reports
    category_id: int = Field(
        foreign_key="expense_categories.id", index=True
    )  # Index cho category queries
    amount: int = Field(index=True)  # Index cho amount-based queries
    vendor: Optional[str] = Field(
        default=None, max_length=255, index=True
    )  # Index cho vendor analysis
    note: Optional[str] = Field(default=None, max_length=500)
    building_id: Optional[int] = Field(
        default=None, index=True
    )  # Index cho building reports
    property_id: Optional[int] = Field(
        default=None, index=True
    )  # Index cho property reports
    allocation_method: str = Field(
        default="direct", max_length=50, index=True
    )  # Index cho allocation analysis
    ...
    class Config:
        indexes = [
            ...
            Index(
                "idx_expense_property_month", "property_id", "month"
            ),  # Property monthly reports
            Index(
                "idx_expense_building_month", "building_id", "month"
            ),  # Building monthly reports
            Index(
                "idx_expense_category_month", "category_id", "month"
            ),  # Category analysis
            Index(
                "idx_expense_vendor_month", "vendor", "month", "amount"
            ),  # Vendor performance
            Index(
                "idx_expense_allocation",
                "allocation_method",
                "building_id",
                "property_id",
            ),  # Allocation queries
...
class RecurringExpense(SQLModel, table=True):
    ...
    class Config:
        indexes = [
            Index(
                "idx_recurring_active", "is_active", "start_month", "end_month"
            ),  # Active recurring expenses
            Index(
                "idx_recurring_schedule", "day_of_month", "is_active"
...
class ExpenseAllocation(SQLModel, table=True):
    ...
    expense_id: int = Field(
        foreign_key="expenses.id", index=True
    )  # Index cho expense lookups
    ...
    class Config:
        indexes = [
            Index(
                "idx_allocation_expense_property", "expense_id", "property_id"
            ),  # Allocation calculations
            Index(
                "idx_allocation_property_month", "property_id", "month"
            ),  # Property monthly allocations
        ]
...
class RoomAssignment(SQLModel, table=True):
    ...
    # Room information
    booked_room: Optional[str] = Field(
        default=None
    )  # Room originally booked (e.g. "203")
    actual_room: Optional[str] = Field(
        default=None
    )  # Room actually occupied (e.g. "303")

    # Revenue attribution
    revenue_attribution: str = Field(
        default="actual_room"
    )  # "booked_room" | "actual_room" | "split"
