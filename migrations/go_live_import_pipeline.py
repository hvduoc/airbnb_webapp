    ...
    try:
        ...
        new_fields = [
            ...
            (
                "row_hash",
                "VARCHAR(64)",
            ),  # Remove UNIQUE constraint for SQLite compatibility
        ]
        ...
        indexes = [
            (
                "idx_booking_source",
                "CREATE INDEX IF NOT EXISTS idx_booking_source ON booking(source)",
            ),
            (
                "idx_booking_channel",
                "CREATE INDEX IF NOT EXISTS idx_booking_channel ON booking(channel)",
            ),
            (
                "idx_booking_imported_at",
                "CREATE INDEX IF NOT EXISTS idx_booking_imported_at ON booking(imported_at)",
            ),
            (
                "idx_booking_ingestion_id",
                "CREATE INDEX IF NOT EXISTS idx_booking_ingestion_id ON booking(ingestion_id)",
            ),
            (
                "idx_booking_row_hash",
                "CREATE INDEX IF NOT EXISTS idx_booking_row_hash ON booking(row_hash)",
            ),
            (
                "idx_booking_source_channel",
                "CREATE INDEX IF NOT EXISTS idx_booking_source_channel ON booking(source, channel)",
            ),
        ]
