    ...
    def create_backup(self) -> str:
        ...
        try:
            backup_file = os.path.join(
                self.backup_dir, f"backup_sqlite_{self.migration_timestamp}.db"
            )
    ...
    def export_data_to_json(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            export_file = os.path.join(
                self.backup_dir, f"exported_data_{self.migration_timestamp}.json"
            )
            ...
            for table in required_tables:
                if table not in data or len(data[table]) == 0:
                    ...
                    validation_results.append(
                        f"✅ Table '{table}': {len(data[table])} records"
                    )

            # Kiểm tra tính toàn vẹn dữ liệu
            if "booking" in data and "property" in data:
                property_ids = {p["id"] for p in data["property"]}
                booking_property_ids = {
                    b["property_id"] for b in data["booking"] if b.get("property_id")
                }

                missing_properties = booking_property_ids - property_ids
                if missing_properties:
                    validation_results.append(
                        f"⚠️ Bookings có property_id không tồn tại: {missing_properties}"
                    )
                else:
            ...
            with Session(target_engine) as session:
                ...
                for table_name in import_order:
                    if table_name in data and table_name in model_mapping:
                        ...
                        logger.info(
                            f"📥 Import table '{table_name}': {len(table_data)} records..."
                        )
                        ...
                        for record in table_data:
                            try:
                                ...
                                logger.warning(
                                    f"⚠️ Lỗi import record trong {table_name}: {e}"
                                )
                                session.rollback()
                                continue
                        ...
                        logger.info(
                            f"✅ Imported {imported_count} records vào '{table_name}'"
                        )
                ...
                for table in tables_to_check:
                    try:
                        ...
                        if source_count == target_count:
                            logger.info(
                                f"✅ Table '{table}': {source_count} = {target_count}"
                            )
                        else:
                            logger.error(
                                f"❌ Table '{table}': Source={source_count} != Target={target_count}"
                            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Chỉ validate, không import"
    )
