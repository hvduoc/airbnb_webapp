...
class VietnameseMonitoringManager:
        ...
        for service, url in self.endpoints.items():
            try:
                response = requests.get(
                    ...
                    else f"{url}/api/health"
                    if service == "grafana"
                    else f"{url}/-/healthy"
                    if service in ["prometheus", "alertmanager"]
                    else f"{url}/app/monitoring"
                    if service == "kibana"
                    else f"{url}/health",
                    timeout=10,
                )
    ...
    def view_logs(self, service: str = "all", follow: bool = False):
        ...
        if service == "all":
            ...
            command = (
                f"docker-compose -f {self.compose_files['monitoring']} logs {service}"
            )
    ...
    def cleanup_monitoring(self):
        ...
        if confirmation.lower() == "yes":
            ...
            self.run_command(
                f"docker-compose -f {self.compose_files['monitoring']} down -v"
            )
    ...
    def setup_kibana_dashboards(self):
        ...
        for attempt in range(max_attempts):
            try:
                response = requests.get(
                    f"{self.endpoints['kibana']}/api/status", timeout=10
                )
                if response.status_code == 200:
                    kibana_ready = True
                    break
        ...
        index_patterns = [
            {"title": "airbnb-logs-application-*", "timeFieldName": "@timestamp"},
            {"title": "airbnb-logs-nginx-*", "timeFieldName": "@timestamp"},
            {"title": "airbnb-logs-postgres-*", "timeFieldName": "@timestamp"},
        ]
