...
def check_brain_health():
    ...
    health["health_percentage"] = round((health["existing_files"] / health["total_files"]) * 100)
...
def load_context_status():
    ...
    if os.path.exists(context_file):
        try:
            ...
            return {
                'exists': True,
                'last_modified': datetime.fromtimestamp(os.path.getmtime(context_file)).strftime("%Y-%m-%d %H:%M"),
                'size': len(content),
    ...
    try:
        ...
        if tasks_file.exists():
            with open(tasks_file, 'r', encoding='utf-8') as f:
                ...
                total_tasks = len(tasks_data.get("active_tasks", [])) + len(tasks_data.get("completed_tasks", []))
        ...
        brain_files = len(list(Path(".brain").glob("**/*.md"))) if Path(".brain").exists() else 0
        daily_logs = len(list(Path(".brain/logs/daily").glob("*.md"))) if Path(".brain/logs/daily").exists() else 0
