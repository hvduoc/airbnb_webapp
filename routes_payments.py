...
async def list_payments(
    ...
    try:
        ...
        for payment in payments:
            response_payments.append(PaymentResponse(
...
async def add_cash_handover(
    ...
    current_user: PaymentUser = Depends(require_role([UserRole.ASSISTANT, UserRole.MANAGER, UserRole.OWNER]))
):
    """
...
async def list_cashflow(
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    current_user: PaymentUser = Depends(require_role([UserRole.MANAGER, UserRole.OWNER]))
):
    """
    ...
    try:
        ...
        for record in cashflow:
            response_cashflow.append(CashHandoverResponse(
    ...
    try:
        ...
        for payment in payments:
            ...
            try:
                ...
                revenue_by_period[key]["total_collected"] += float(payment.get("Amount Collected (VND)", 0))
                revenue_by_period[key]["total_due"] += float(payment.get("Amount Due (VND)", 0))
...
async def get_collector_performance(
    ...
    try:
        ...
        for payment in payments:
            ...
            collector_performance[collector]["total_collected"] += float(payment.get("Amount Collected (VND)", 0))
            collector_performance[collector]["total_due"] += float(payment.get("Amount Due (VND)", 0))
        ...
        return {
            "collectors": sorted_data,
            "total_collectors": len(sorted_data)
        }
