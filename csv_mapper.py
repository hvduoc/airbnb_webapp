...
class BookingValidationModel(BaseModel):
    ...
    def validate_amount(cls, v):
        ...
        if v < 10000 or v > 10000000:  # 10k to 10M VND
            raise ValueError(
                f"Amount {v:,} VND is outside reasonable range (10k-10M VND)"
            )
        return v
...
class CSVMapper:
    ...
    def _map_airbnb_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for csv_key, db_field in airbnb_mapping.items():
            if (
                csv_key in row
                and row[csv_key] is not None
                and not pd.isna(row[csv_key])
            ):
    ...
    def _parse_amount(self, amount_str: str) -> int:
        ...
        # Handle multipliers first (including space-separated)
        multiplier = 1
        for suffix, mult in self.amount_config["multipliers"].items():
            suffix_lower = suffix.lower()
            # Check both direct suffix and space-separated
            if amount_str.endswith(suffix_lower):
                multiplier = mult
                amount_str = amount_str[: -len(suffix_lower)].strip()
                break
            elif amount_str.endswith(" " + suffix_lower):
                multiplier = mult
                amount_str = amount_str[: -(len(suffix_lower) + 1)].strip()
                break
...
class ImportPipeline:
    ...
    def process_csv(
        self, df: pd.DataFrame, source: str, channel: str
    ) -> Tuple[List[Dict], str]:
        """
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if row_hash in seen_hashes:
                    ...
                    self._log_error(
                        idx, row.to_dict(), "Duplicate row in current batch"
                    )
                    continue
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": round(
                self.stats["valid"] / max(self.stats["total"], 1) * 100, 2
            ),
