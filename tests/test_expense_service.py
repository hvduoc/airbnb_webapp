...
from models import User, Expense, ExpenseCategory, Property
...
class MockExpense:
    """Mock Expense object"""
    def __init__(self, id=1, amount=500000, category_id=1, property_id=1,
                 vendor="Test Vendor", date="2025-10-03", month="2025-10"):
        self.id = id
        self.amount = amount
...
class MockQueryResult:
    ...
    def __init__(self, property_id=1, property_name="Test Property", 
                 expense_count=5, total_amount=2500000, avg_amount=500000):
        self.property_id = property_id
        self.property_name = property_name
...
class TestExpenseService:
    ...
    def test_create_expense_success(self):
        ...
        with patch.object(service, 'require_permission') as mock_permission:
            with patch.object(service, 'apply_property_filter') as mock_filter:
                ...
                service.db.exec.return_value.first.side_effect = [mock_category, mock_property]
    ...
    def test_create_expense_permission_denied(self):
        ...
        with patch.object(service, 'require_permission', side_effect=HTTPException(status_code=403, detail="Insufficient permission: expense/create")):
            # Act & Assert
    ...
    def test_create_expense_property_access_denied(self):
        ...
        # Arrange
        staff_user = MockUser(id=3, roles="staff", property_ids="1,2")  # No access to property 3
        ...
        with patch.object(service, 'require_permission'):
            with patch.object(service, 'apply_property_filter') as mock_filter:
                ...
                mock_filter.return_value = Mock()
                service.db.exec.return_value.first.side_effect = [mock_category, None]  # No property access
    ...
    def test_list_expenses_property_filtering(self):
        ...
        mock_expenses = [
            type('obj', (object,), {
                'id': 1, 'amount': 500000, 'vendor': 'Vendor A', 'note': 'Note 1',
                'date': '2025-10-01', 'month': '2025-10', 'allocation_method': 'direct',
                'created_at': datetime.utcnow(), 'category_name': 'Maintenance',
                'property_name': 'Property 1'
            })(),
            type('obj', (object,), {
                'id': 2, 'amount': 300000, 'vendor': 'Vendor B', 'note': 'Note 2',
                'date': '2025-10-02', 'month': '2025-10', 'allocation_method': 'direct',
                'created_at': datetime.utcnow(), 'category_name': 'Utilities',
    ...
    def test_summary_by_property_aggregation(self):
        ...
        mock_results = [
            MockQueryResult(
                property_id=1, property_name="Property 1",
                expense_count=10, total_amount=5000000, avg_amount=500000
            ),
            MockQueryResult(
                property_id=2, property_name="Property 2", 
                expense_count=8, total_amount=3200000, avg_amount=400000
            ),
            MockQueryResult(
                property_id=3, property_name="Property 3",
                expense_count=5, total_amount=1500000, avg_amount=300000
            )
        ]
    ...
    def test_no_user_context_denied(self):
        ...
        # Act & Assert
        with pytest.raises(HTTPException) as exc_info:
            service.create_expense({"amount": 500000, "category_id": 1, "date": "2025-10-03"})
