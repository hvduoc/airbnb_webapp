...
from csv_mapper import CSVMapper, ImportPipeline, ValidationError
...
class TestCSVMapper(unittest.TestCase):
    ...
    def test_parse_amount_with_multipliers(self):
        ...
        for amount_str, expected in test_cases:
            ...
            self.assertEqual(result, expected, f"Failed to parse {amount_str}: got {result}, expected {expected}")
...
Valid Guest,1,15/10/2025,16/10/2025,1.000.000đ"""
        ...
        self.assertGreaterEqual(self.pipeline.stats['invalid'], 1)  # At least one invalid
