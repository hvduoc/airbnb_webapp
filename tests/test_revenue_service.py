...
class MockBooking:
    """Mock Booking object"""
    def __init__(self, id=1, property_id=1, total_amount=1000000, 
                 checkin_date=date(2025, 10, 1), checkout_date=date(2025, 10, 3),
                 status="confirmed"):
        self.id = id
...
class MockQueryResult:
    ...
    def __init__(self, property_id=1, property_name="Test Property", 
                 total_bookings=5, total_revenue=5000000, avg_booking_value=1000000,
                 first_booking=date(2025, 10, 1), last_booking=date(2025, 10, 31)):
        self.property_id = property_id
        self.property_name = property_name
...
class TestRevenueService:
    ...
    def test_admin_revenue_access_allowed(self):
        ...
        # Mock query results
        mock_results = [
            MockQueryResult(property_id=1, property_name="Property 1", total_revenue=3000000),
            MockQueryResult(property_id=2, property_name="Property 2", total_revenue=2000000)
        ]
    ...
    def test_staff_revenue_access_denied_insufficient_role(self):
        ...
        with patch.object(service, 'require_permission', side_effect=HTTPException(status_code=403, detail="Insufficient permission: revenue/read")):
            # Act & Assert
    ...
    def test_property_filtering_correctness(self):
        ...
        mock_results = [
            MockQueryResult(property_id=1, property_name="Accessible 1", total_revenue=1000000),
            MockQueryResult(property_id=2, property_name="Accessible 2", total_revenue=2000000)
        ]
        service.db.exec.return_value.all.return_value = mock_results
    ...
    def test_revenue_aggregation_correctness(self):
        ...
        mock_results = [
            MockQueryResult(
                property_id=1, property_name="High Revenue Property", 
                total_bookings=10, total_revenue=5000000, avg_booking_value=500000
            ),
            MockQueryResult(
                property_id=2, property_name="Medium Revenue Property",
                total_bookings=8, total_revenue=3200000, avg_booking_value=400000
            ),
            MockQueryResult(
                property_id=3, property_name="Low Revenue Property",
                total_bookings=5, total_revenue=1500000, avg_booking_value=300000
            )
        ]
        service.db.exec.return_value.all.return_value = mock_results
    ...
    def test_date_filtering_applied(self):
        ...
        with patch.object(service, 'apply_property_filter', side_effect=lambda q, attr: q):
            # Act
            result = service.revenue_by_property(start_date=start_date, end_date=end_date)
    ...
    def test_include_cancelled_bookings_parameter(self):
        ...
        with patch.object(service, 'apply_property_filter', side_effect=lambda q, attr: q):
            ...
            result_without_cancelled = service.revenue_by_property(include_cancelled=False)
        
        # Assert parameter recorded
        assert result_with_cancelled["summary"]["period"]["include_cancelled"] == True
        assert result_without_cancelled["summary"]["period"]["include_cancelled"] == False
