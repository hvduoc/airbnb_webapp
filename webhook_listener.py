...
async def root():
    return {
        "message": "Airbnb GitHub Webhook Listener - Home Server",
        "status": "running",
    }
...
async def github_webhook(request: Request):
    try:
        ...
        print(
            f"📂 Repository: {event_data.get('repository', {}).get('name', 'unknown')}"
        )
...
async def brain_status():
    ...
    try:
        status = {"brain_dir_exists": os.path.exists(BRAIN_DIR), "files": []}
