...
class AISessionManager:
    ...
    def start_session(self, agent_name="AI Agent", goals=None):
        ...
        health_result = subprocess.run(
            ["python", "scripts/health_check.py"], capture_output=True, text=True
        )
    ...
    def list_tasks(self):
        ...
        for task in tasks:
            status = "✅" if task.get("completed") else "🔄"
            priority = {"high": "🔥", "medium": "📋", "low": "⬇️"}.get(
    ...
    def _prompt_session_summary(self, session_data, completed_tasks):
        ...
        if completed_tasks:
            summary = f"Completed {len(completed_tasks)} tasks: " + ", ".join(
                [t["title"] for t in completed_tasks]
            )
        else:
