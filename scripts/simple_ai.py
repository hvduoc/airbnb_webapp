...
class SimpleSessionManager:
    ...
    def start_session(self, agent_name="AI Agent"):
        ...
        # 3. Set session goals
        high_priority = [
            t for t in tasks if t.get("priority") == "high" and not t.get("completed")
        ][:2]
    ...
    def end_session(self, summary=None):
        ...
        try:
            subprocess.run(
                ["python", "scripts/context_update.py"], capture_output=True, timeout=30
            )
    ...
    def list_tasks(self):
        ...
        for task in tasks:
            status = "[DONE]" if task.get("completed") else "[TODO]"
            priority = {"high": "[HIGH]", "medium": "[MED]", "low": "[LOW]"}.get(
                task.get("priority", "medium"), "[MED]"
            )
...
def main():
    ...
    parser.add_argument(
        "command", choices=["start", "end", "add-task", "complete-task", "list-tasks"]
    )
