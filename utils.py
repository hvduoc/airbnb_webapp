...
def get_room_mapping_preview(df, room_mapping_data=None):
    ...
    for index, row in df.head(10).iterrows():
        ...
        preview.append(
            {"original": original, "mapped_name": mapped_name, "mapped": is_mapped}
        )
...
def get_properties_stats(month: str, building_id: int | None = None):
    ...
    end_date = (start_date.replace(day=1) + timedelta(days=32)).replace(
        day=1
    ) - timedelta(days=1)
    ...
    with Session() as session:
        ...
        for prop in properties:
            ...
            sold_nights = sum(
                (min(b.end_date, end_date) - max(b.start_date, start_date)).days
                for b in prop_bookings
            )
            available_nights = (
                end_date - start_date
            ).days + 1  # Total nights in the month
