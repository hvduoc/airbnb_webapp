...
async def login_page(request: Request, error: str = None):
    ...
    return templates.TemplateResponse(
        "auth_login.html", {"request": request, "error": error}
    )
...
async def refresh_token(
    request: Request, refresh_token: str = Form(...), db: Session = Depends(get_session)
):
...
async def list_users(
    current_user: User = Depends(require_admin), db: Session = Depends(get_session)
):
...
@router.get("/check")
async def check_auth_status(current_user: User = Depends(get_optional_current_user)):
