...
async def go_live_upload(
    ...
    for upload_file in files:
        ...
        try:
            ...
            for row_data in valid_rows:
                ...
                # Create new booking
                booking = Booking(**{k: v for k, v in row_data.items() if k != 'row_hash'})
    ...
    total_stats['processing_time'] = (datetime.utcnow() - processing_start).total_seconds()
...
async def validate_csv(
    ...
    for upload_file in files:
        ...
        try:
            ...
            summary['sample_valid_rows'] = valid_rows[:3]  # First 3 valid rows as example
        ...
        'imports': [
            {
                ...
                'success_rate': round((log.rows_inserted / max(log.rows_total, 1)) * 100, 2),
...
async def get_import_stats(db: Session = Depends(get_db)):
    ...
    return {
        ...
        'stats': {
            ...
            'success_rate': round((total_rows_inserted / max(total_rows_processed, 1)) * 100, 2),
            'sources': sources,
            'channels': channels
        }
    }