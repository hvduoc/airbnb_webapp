...
def check_database_schema():
    ...
    print("\n=== EXTRACHARGE SCHEMA ===")
    try:
        schema = cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='extracharge'").fetchone()
        if schema:
            print(schema[0])
        else:
    ...
    try:
        schema = cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='extra_charges'").fetchone()
        if schema:
            print(schema[0])
        else:
    ...
    try:
        schema = cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='expense_categories'").fetchone()
        if schema:
            print(schema[0])
        else:
