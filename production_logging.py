...
class AirbnbProductionLogger:
    ...
    def log_performance(
        ...
        if duration_ms > 1000:  # Slow operation > 1s
            performance_logger.warning(
                f"Slow operation: {operation} ({duration_ms}ms)", extra=extra
            )
        else:
            performance_logger.info(
                f"Performance: {operation} ({duration_ms}ms)", extra=extra
            )

    def log_database_query(
        self, query: str, duration_ms: float, rows_affected: int = 0
    ):
    ...
    def log_error(self, error: Exception, context: Optional[Dict[str, Any]] = None):
        ...
        error_logger.error(
            f"Application Error: {str(error)}", exc_info=True, extra=extra
        )
...
def log_performance_context(
    logger: AirbnbProductionLogger, operation: str, endpoint: Optional[str] = None
):
    ...
    try:
        ...
        logger.log_performance(
            operation=operation, duration_ms=duration_ms, endpoint=endpoint
        )
