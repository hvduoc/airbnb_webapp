...
class CanaryMonitor:
    ...
    def check_thresholds(self):
        ...
        if current_metrics["error_rate"] > self.thresholds["max_error_rate"]:
            violations.append(
                f"Error rate: {current_metrics['error_rate']:.2%} > {self.thresholds['max_error_rate']:.2%}"
            )

        if current_metrics["p95_latency"] > self.thresholds["max_p95_latency"]:
            violations.append(
                f"P95 latency: {current_metrics['p95_latency']:.1f}ms > {self.thresholds['max_p95_latency']}ms"
            )

        server_error_rate = self.metrics["server_errors"] / max(
            1, self.metrics["total_requests"]
        )
        if server_error_rate > self.thresholds["max_5xx_rate"]:
            violations.append(
                f"5xx rate: {server_error_rate:.2%} > {self.thresholds['max_5xx_rate']:.2%}"
            )
    ...
    def print_status(self):
        ...
        print("\n" + "=" * 60)
        print("🚨 CANARY MONITORING - v1.3.0 (5% Traffic)")
        print(f"⏱️  Runtime: {elapsed}")
        print(
            f"📊 Requests: {metrics['total_requests']} | Success: {metrics['successful_requests']}"
        )
